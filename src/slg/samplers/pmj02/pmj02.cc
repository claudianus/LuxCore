 ...
 * for LuxCoreRender (include paths flattened; only the plain PMJ02 path).
 */
#include "pmj02.h"
...
class SampleSet {
 ...
 private:
  ...
  // Number of candidates to use for best-candidate sampling (1 = plain PMJ02).
...
Point SampleSet::GetCandidateSample(const vector<int>& valid_x_strata,
  ...
  int x_strata_index = valid_x_strata[UniformInt(0, static_cast<int>(valid_x_strata.size())-1)];
  int y_strata_index = valid_y_strata[UniformInt(0, static_cast<int>(valid_y_strata.size())-1)];
...
void SampleSet::GenerateNewSample(const int sample_index,
  ...
  // num_candidates_ is 1: plain PMJ02, no best-candidate search.
...
void SampleSet::UpdateStrata(const int sample_index) {
  ...
  for (int i = 0, strata_n_cols = n_, strata_n_rows = 1;
    ...
    int x_pos = static_cast<int>(sample.x * strata_n_cols);
    int y_pos = static_cast<int>(sample.y * strata_n_rows);
    strata_[i][y_pos*strata_n_cols + x_pos] = true;
  }

  const int x_pos = static_cast<int>(sample.x * dim_), y_pos = static_cast<int>(sample.y * dim_);
 ...
 */
std::unique_ptr<Point[]> GenerateSamples(const int num_samples) {
  ...
  while (n < num_samples) {
    ...
    for (int i = 0; i < n && n+i < num_samples; i++) {
      ...
      int x_pos = static_cast<int>(sample.x * sample_set.dim());
      int y_pos = static_cast<int>(sample.y * sample_set.dim());
    ...
    const std::vector<std::pair<int, int>> sub_quad_choices =
        GetSubQuadrantsSwapXOrY(sample_set.samples(), sample_set.dim());
